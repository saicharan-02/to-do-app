...
function Home() {
  ...
  const requestOptions = {
    ...
    body: JSON.stringify({ task: taskData, creds: { id: 1 } }),
  };
  ...
  function fetchTasks() {
    fetch(`http://localhost:8081/task/viewAll?id=1`, {
      method: "GET",
      headers: { "Content-Type": "application/json" },
    })
  ...
  const handleOnSubmit = (e) => {
    e.preventDefault();
    fetch("http://localhost:8081/task/addTask", requestOptions)
      .then((response) => response.json())
      .then((data) => {
        setTasks([...tasks, data]);
      });
  ...
  return (
    ...
    <div className="flex flex-col justify-center items-center min-h-screen max-w-screen">
      ...
      <div>
        {tasks.map((task) => (
          <Task key={task.taskId} indTask={task} index={++count} />
        ))}
      </div>
    </div>
  );
